...
object BSO{
  ...
  def loss_CRF(
    ...
    var subLogs = List[Expression]()

    var currGoldState : PredictionState = null
    while(goldLeftover.nonEmpty && !stop){
      currGoldState = goldLeftover.head
...
//        assert(currGoldState.isFinished)
        val goldPos = beam.indexOf(currGoldState)
        if(goldPos>=0){
          if(beam.size>1){
            subLogs ::= logSoftmax(concat(beam.map(_.scoreExp):_*))(goldPos)
          }
        }else{
          subLogs ::= logSoftmax(concat((currGoldState::beam).map(_.scoreExp):_*))(0)
        }
      }else if(!beam.contains(currGoldState)){
