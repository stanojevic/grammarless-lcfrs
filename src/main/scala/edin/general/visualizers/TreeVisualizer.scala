...
object TreeVisualizer {
  ...
  case class SimpleTreeNode(
                             ...
                             var arcColor  : Color.Color          = Color.BLACK,
                             var arcStyle  : ArcStyle.ArcStyle    = ArcStyle.SOLID,
    ...
    def visualize(graphLabel:String="", fileType:String="pdf") : Unit = {
      val file = File.createTempFile(s"visual__________${graphLabel.replace(" ", "_")}__________", "."+fileType, null)
      file.deleteOnExit()
  ...
  def toDotString(mainNode:SimpleTreeNode, graphLabel:String) : String = {
    def toDotStringRec(node:SimpleTreeNode, nodeId:String) : (String, List[String]) = {
      ...
      node.children.zipWithIndex.foreach{ case (child, index) =>
        ...
        val style = s"color=${child.arcColor},style=${child.arcStyle}"
    ...
    for( (nodeId, node) <- (terms zip mainNode.allTerminals).sortBy(_._2.position) ){
