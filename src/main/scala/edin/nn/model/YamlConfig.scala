...
class YamlConfig(val conf:AnyRef){
  ...
  def contains(key:String) : Boolean =
    conf!=null && conf.asInstanceOf[Map[String, AnyRef]].contains(key)
  ...
  def apply(index:Int) : YamlConfig = {
    val conf = this.conf.asInstanceOf[List[AnyRef]]
    require(conf!=null && conf.size>index, s"list doesn't have [$index] index")
    new YamlConfig(conf(index))
  }

  def apply(key:String): YamlConfig = {
    val conf = this.conf.asInstanceOf[Map[String, AnyRef]]
    require(conf!=null && conf.contains(key), s"key $key is missing")
    new YamlConfig(conf(key))
  }
